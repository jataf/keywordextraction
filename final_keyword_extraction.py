...
def replace_with_span(word, tag):
    tagToTitle = {
        "daten": "Datentyp",
        "variable": "Variable",
        "methode": "Methode",
    ...
    return "<span title='" + tagToTitle[tag] + "' class=\"spacy_" + tag.lower() + "\">" + word + "</span>"
...
body = soup.find('body')
for script in body.find_all('script', src=False):
    script.decompose()
textNodes = body.findAll(text=True)
...
for textNode in textNodes:
    original = textNode
    words =re.findall(r"[\w']+", textNode)
    for word in words:
        if word in wordToClass and word != '\u2192':
            word_with_class = replace_with_span(word, wordToClass[word])
            if textNode == word:
                textNode = word_with_class
            # Ersetzt alle Vorkommnisse des Wortes im Textabschnitt mit dem gebauten Span
            textNode = textNode.replace(word + " ", word_with_class + " ")
            textNode = textNode.replace(word + ",", word_with_class + ",")
            textNode = textNode.replace(" " + word, " " + word_with_class)

    # Ersetzt den ursprünglichen Textabschnitt mit dem ergänzten
    body_str = body_str.replace(original, textNode)
